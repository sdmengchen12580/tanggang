...
public class JazzyViewPager extends ViewPager {
	...
	public JazzyViewPager(Context context, AttributeSet attrs) {
		...
		setFadeEnabled(ta.getBoolean(R.styleable.JazzyViewPager_jvp_fadeEnabled, false));
...
//}
	...
	protected void animateOutline(View left, View right) {
		...
		if (mState != State.IDLE) {
			if (left != null) {
				manageLayer(left, true);
				((OutlineContainer)left).setOutlineAlpha(0.0f);
			}
			if (right != null) {
				manageLayer(right, true);
				((OutlineContainer)right).setOutlineAlpha(0.0f);
			}
		} else {
			if (left != null)
