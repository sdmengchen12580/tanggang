...
public class QuestionAdapter extends RecyclerView.Adapter implements View.OnClickListener{
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof DiscussViewHolder) {
            ...
            discussViewHolder.tvTime.setText(CommonUtil.conver2Date(CommonUtil.convertMilliSec(resourcesBean.getLatestPostTime())).substring(2, 16));
            ...
            if (resourcesBean.getIsStick() != null) {
                if (resourcesBean.getIsStick().equals("1")) {
                    discussViewHolder.llStickView.setVisibility(View.VISIBLE);
                } else {
                    discussViewHolder.llStickView.setVisibility(View.GONE);
                }
            }
            if (resourcesBean.getIsElite() != null) {
                if (resourcesBean.getIsElite().equals("1")) {
                    discussViewHolder.tvIsElite.setVisibility(View.VISIBLE);
                    discussViewHolder.tvContent.setText(String.format("                %s", resourcesBean.getTitle()));
                } else {
                    discussViewHolder.tvIsElite.setVisibility(View.GONE);
                    discussViewHolder.tvContent.setText(String.format("         %s", resourcesBean.getTitle()));
                }
            }
